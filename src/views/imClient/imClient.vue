...
<template lang="pug">
     .app-main
        chatHeader
        .imClient-inner
            //- header.imClient-header
            //-     .name-wrapper.position-v-mid
            //-         span(v-if="chatInfoEn.chatState == 'robot'") Vue在线客服-访客端
            //-         span(v-else-if="chatInfoEn.chatState == 'agent'") 您正在与客服{{serverChatEn.serverChatName}}对话
            //-     .opr-wrapper.position-v-mid
            ...
            //-             i.fa.fa-envelope-o(@click="showLeaveDialog()")
            //-         el-tooltip(content="结束会话" placement="bottom" effect="light")
            //-             i.fa.fa-close(@click="closeChat()")
            main.imClient-main
                //  聊天框
                .item.imClientChat-wrapper
                    //  聊天记录
                    common-chat(ref="common_chat" :chatInfoEn="chatInfoEn" :oprRoleName=" 'client'" @sendMsg="sendMsg" @showRateDialog="showRateDialog" @chatCallback="chatCallback")
        //  转接客服dialog
        ...
        el-dialog(:visible.sync="leaveDialogVisible" :close-on-press-escape="false")
            im-leave(ref="im_leave")
</template>

<script >
import commonChat from '@/components/common/common_chat.vue';
import header from '@/views/imClient/header.vue';
...
export default {
    ...
    methods: {
        ...
        regSocket: function(serverChatId) {
            this.$data.socket = require('socket.io-client')('http://localhost:3001'); // http://localhost:3001 // http://192.168.80.67:3000/mock/34/im/user
...
.imClientInfo-wrapper .imClientInfo-notice-wrapper .imClientInfo-item-header,
.imClientInfo-wrapper .imClientInfo-faq-wrapper .imClientInfo-item-header
  font-weight bolder
  font-size 16px
  color #1072b5
  padding 10px 15px 0
